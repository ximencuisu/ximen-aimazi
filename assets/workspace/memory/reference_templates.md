...
| `CHAPTER-TEMPLATE.md` | `assets/` | 男频章节模板，含写作前读取确认硬门控与文风圣经检查 | 生成正文时套用 |
| `CHAPTER-TEMPLATE.female.md` | `assets/` | 女频（言情）章节模板，含写作前读取确认硬门控与文风圣经检查 | 女频小说时使用 |
| `PROMPT-TEMPLATE.md` | `assets/` | 提示词模板（完整版+快速版） | 生成创作提示词时 |
| `LEARNINGS-TEMPLATE.md` | `assets/` | 记忆文件模板 | 初始化新.learnings文件时 |
| `workspace/` | `assets/` | 项目级工作区模板（`.learnings` / `SESSION.md` / `memory/project_*.md` / `设定/` / `大纲/` / `追踪/`，含参考调用记录、大纲、细纲、伏笔、时间线、冻结清单） | `init-novel` 初始化或 `--clean` 重置时 |

## 参考文档

| 文档 | 位置 | 用途 |
|------|------|------|
| `anti-ai-writing.md` | `references/` | 去AI味详细指南（绝对零度写作） |
| `quality-check.md` | `references/` | 质量评分标准 |
