...
metadata:
  ...
  tags: "novel, fiction, creative-writing, chinese, 爽文, 小说生成, 创作助手, 去AI味, 细纲, 文风配置, 写作风格, 擦边描写, 情感线, 番茄小说, 热榜选材, 同人创作"
---
...
```
联网选材 → 用户方向 → 灵感生成 → 世界观构建 → 人物塑造 → 创作参数配置
                                                                  ↓
...
**目标**：先选定题材方向，再检索该题材热榜作品，给出选材建议

### 交互流程

#### 步骤1：选择题材方向

**使用 `question` 工具询问用户题材选择**：
...
| 选项 | 题材 | 分类ID | 特点 |
|------|------|--------|------|
| A | 都市修真 | 1141 | 现代背景+修真元素，低调装逼 |
| B | 都市高武 | 1140 | 现代背景+武道，格斗竞技 |
| C | 都市日常 | 8 | 现代都市、生活、职场、情感 |
| D | 传统玄幻 | 261 | 异世界、修炼体系、升级打怪 |
| E | 东方仙侠 | 124 | 修仙、渡劫、宗门、仙器法宝 |
| F | 历史脑洞 | 1014 | 历史+创意设定，脑洞大开 |
| G | 悬疑脑洞 | 273 | 推理、解谜、反转、烧脑 |
| H | 悬疑灵异 | 27 | 鬼怪、灵异事件、惊悚 |
| I | 玄幻脑洞 | 263 | 玄幻+创新设定，打破套路 |
| J | 都市脑洞 | 258 | 现代+奇幻设定，脑洞向 |
| K | 科幻末世 | 272 | 丧尸、废土、异能、科技文明 |
| L | 游戏体育 | 539 | 电竞、体育竞技、游戏世界 |
| M | 历史古代 | 262 | 穿越古代、宫廷、争霸 |
| N | 西方奇幻 | 257 | 魔法、龙、中世纪、精灵矮人 |
| O | 抗战谍战 | 751 | 战争、间谍、情报、热血 |
| P | 战神赘婿 | 504 | 入赘、隐藏身份、打脸逆袭 |
| Q | 都市种田 | 746 | 田园生活、致富、悠闲 |
| R | 动漫衍生 | 718 | 同人、二次元、动漫世界 |
| S | 男频衍生 | 1016 | 其他男性向题材 |
| 自定义 | - | - | 输入你自己的题材想法 |

#### 步骤2：检索对应题材排行榜

**根据用户选择，使用 `webfetch` 抓取对应分类排行榜**：

```
URL格式：https://fanqienovel.com/rank/1_1_*

示例：
- 选择"都市修真" → https://fanqienovel.com/rank/1_1_1141
- 选择"传统玄幻" → https://fanqienovel.com/rank/1_1_261
```
...
| 事件 | 记录到 | 时机 |
|------|--------|------|
| 新角色出场 | `CHARACTERS.md` | 生成完毕后立即 |
| 角色状态变化 | `CHARACTERS.md` | 更新对应条目 |
| 新地点出现 | `LOCATIONS.md` | 生成完毕后立即 |
| 关键情节发生 | `PLOT_POINTS.md` | 生成完毕后立即 |
| 质量问题 | `ERRORS.md` | 失败后立即 |

### 读取时机

**每次生成新章节前必须读取**所有记忆文件，确保：
- 不会让已死角色复活
- 不会把地点写错
- 不会忘记伏笔
- 不会重复情节
...
| 场景 | 操作 |
|------|------|
| 开始创作 | 系统抓取番茄热榜 → 用户选择题材 → 进入灵感生成 |
| 选题材参考 | 系统自动抓取 https://fanqienovel.com/rank/1_1_1141 |
| 选择题材方向 | 19种预设题材 + 自定义 |
| 用户提供方向 | 执行提示词生成流程 |
